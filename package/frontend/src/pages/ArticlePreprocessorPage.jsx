  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <WorkspaceHeader
        rightContent={(
          <div className="hidden items-center gap-2 sm:flex">
            {usage && <span className="text-xs text-slate-500">使用量 {usage.used}/{usage.limit}</span>}
            {selectedSpec && (
      ...
      <main className="max-w-7xl mx-auto px-4 py-6">
        <div className="mb-5">
          <h1 className="text-xl font-semibold text-slate-950">内容预处理</h1>
          <p className="mt-1 text-sm text-slate-500">识别文章结构并整理段落，为格式检查和排版生成准备内容。</p>
        </div>
        {/* Workflow indicator */}
